#!/usr/bin/env pwsh
# Check Test113c CKL Results for Session #32 Batch 2 Functions

$cklPath = "d:\Dropbox\IT Docs\Scripts\VatesVMS-Evaluate-STIG\v1.2507.6_Mod4VatesVMS_OpenCode\Evaluate-STIG\.Mods_by_Kismet\Test\Results\XO1\Checklist\XO1_XO_WebSRG_V4R4_20260203-213803.ckl"

Write-Host "=" * 80
Write-Host "Session #32 Batch 2 - Test113c CKL Results Verification"
Write-Host "=" * 80
Write-Host ""
...
foreach ($vulnId in $batch2VulnIds) {
    ...
    if ($vuln) {
        ...
        $hasError = $findingDetails -match '\*\*\* Scan-.*Failed\.\s\*\*\*|\$Result\.Status is null'
