# Xen Orchestra Automated Compliance Implementation Report
...
This report presents the results of a comprehensive security compliance assessment of Xen Orchestra, conducted as part of the effort to approve the Vates Virtualization Management Stack (Xen Orchestra + XCP-ng) for use in secure and/or air-gapped environments.

There are currently **no official security compliance benchmarks or automated scanning profiles** for Xen Orchestra or XCP-ng. To address this gap, we developed a custom automated scanning framework with purpose-built modules that map applicable Security Requirements Guides (SRGs) and existing security baselines to the Xen Orchestra platform.

### Assessment Scope

| Module | Security Baseline Applied | Rules | Status |
|--------|--------------------------|-------|--------|
| Application Security (ASD) | Application Security and Development Baseline V6R4 | 286 | 100% Automated |
...
| **Not Reviewed** | 0 | 0 |
| **Errors** | 0 | 0 |
| **Checklist Validation** | All Passed | All Passed |

**Note on EvalScores:** These scores reflect honest, automated assessment results. Many Open findings represent genuine product gaps (FIPS cryptography, mandatory login banner) or features that exist but were not detected by our scanning framework at the time of assessment (MFA/2FA via SAML/OIDC). Others require configuration guidance or organizational policy decisions. The scores are not inflated — every finding is backed by automated evidence in the generated compliance checklists (CKL/CKLB files).
...
**Baseline:** Application Security and Development Baseline V6R4
**Rules:** 286
...
2. **Provide LDAP/AD integration guidance** as the recommended authentication method for regulated environments — this delegates password hashing to FIPS-validated directory services
...
**Finding:** Xen Orchestra has **no built-in mechanism** to display a mandatory notice and consent banner before granting access. The login page does not present any banner text, and there is no acknowledgment/consent mechanism.
...
1. **Near-term (nginx reverse proxy):** Publish an nginx configuration template that implements a consent page before the XO login. XOA already ships with nginx — this could be a configuration addition. Template design:
   ...
   - Nginx serves consent page with the organizational banner and "I Agree" button
...
| Topic | Specific Guidance Needed for Compliance |
|-------|---------------------------------------|
| **Mandatory consent banner** | nginx configuration template for mandatory notice and consent |
...
| **Certificate management** | Replace self-signed certificates with organization-signed PKI certificates |
...
| **Kernel parameters** | Recommended sysctl settings for security compliance requirements |
| **Smart card / hardware token integration** | Step-by-step SAML/OIDC configuration for smart card authentication |
| **FIPS 140-2** | Cryptographic module configuration and compliance status |

**Recommended Actions for Vates:** Expand the existing hardening guide (or publish a compliance supplement) with the specific technical procedures listed above. The existing guide provides an excellent framework — adding compliance-specific sections would make it a complete reference.
...
| No mandatory consent banner | 5 | Vates (feature) + Site (nginx config) |
...
| **With compliance-specific guide supplement** (346 Opens) | ~175-230 man-hours | 4-6 weeks |
| **If Vates resolves product gaps + scanner MFA fix** (~50 fewer Opens) | ~150-350 man-hours | 4-9 weeks |
| **Best case** (compliance guide + product fixes, ~250 Opens) | ~125-165 man-hours | 3-4 weeks |

For comparison, a sysadmin hardening a **VMware ESXi host** with its official security baseline and automated scanning benchmark can achieve compliance in approximately **40-80 man-hours** — the automated scanning tool handles most checks, the vendor baseline provides exact fix actions, and the community knowledge base covers edge cases.
...
| Automated Scanning Benchmark | None | Yes (automated scanning) |
...
1. **A compliance supplement to the existing hardening guide** could cut per-finding effort by 50-60%, saving each deploying organization hundreds of hours. The existing v0.1 guide is an excellent foundation — adding compliance-specific procedures (PAM, FIPS, smart card, consent banner) would close the gap
2. **Resolving product gaps** (FIPS, banner, TLS) removes findings that no sysadmin can fix — these are currently hard blockers that require remediation plans. Note: MFA was previously listed here but XO already supports 2FA/SAML/OIDC — this is a scanner detection issue being resolved
...
1. **Publish a compliance supplement to the existing Hardening Guide** — the v0.1 guide (May 2024) is an excellent foundation; add compliance-specific procedures for PAM, FIPS, consent banner, smart card authentication, and session management (see Section 4.5 gap table)
...
3. **Publish nginx consent banner template** for regulated deployments (can be done without product code changes)
4. **Document TLS hardening steps** to disable TLS 1.1 and enforce TLS 1.2+
5. **Document smart card + SAML/OIDC integration** as the recommended MFA architecture for regulated environments — the 2FA capability already exists (Section 5.4 of the hardening guide), but compliance teams need step-by-step configuration for smart card authentication
...
|----------|----------|---------|
| Compliance Supplement to Hardening Guide | Critical | Compliance-specific procedures (PAM, FIPS, banner, smart card) |
...
- **Tool:** Custom automated compliance scanning framework with purpose-built modules
...
- **Output:** Compliance checklist files (CKL/CKLB) + XML Summary Reports
...
- **Checklist validation** — all generated checklists pass schema validation
...
|----------|--------|---------|
| Compliance Checklists | CKL, CKLB | Per-rule findings with automated evidence |
...
*This report was generated based on automated security compliance scans performed on March 3, 2026. All findings are backed by machine-generated evidence in the accompanying CKL/CKLB checklist files.*
