...
**Purpose:** Track compliance blockers, missing packages/features, and items requiring Vates team input for STIG compliance approval (IATT/ATO).

**Last Updated:** March 15, 2026
**Document Owner:** Kismet Agbasi
**Target:** Regulated Environment Approval (IATT for PoC or Full ATO for Production)
...
**Requirement:** All cryptographic operations must use FIPS 140-2 validated modules.
...
1. **LDAP/AD Integration (Recommended):** Delegate authentication to FIPS-validated directory services (Active Directory, LDAP with FIPS-validated TLS). This is the most practical path for regulated environments.
...
- Document LDAP/AD integration as recommended authentication method for regulated environments
...
**Requirement:** Minimum TLS 1.2. TLS 1.0 and TLS 1.1 must be disabled.
...
**Requirement:** All audit log timestamps must use UTC or GMT.
...
- Recommend UTC as default timezone for regulated deployments
...
**Requirement:** All audit logs must be forwarded to an approved centralized audit server (SIEM/syslog server).
...
- CAC/PIV smart cards (recommended for regulated environments)
...
- Consider native MFA support as a feature request for regulated deployments
...
**Requirement:** Minimum 15 characters. Complexity requirements enforced.
...
### 1.8 No Mandatory Notice and Consent Banner
...
**Finding:** Xen Orchestra has **no built-in mechanism** to display a mandatory notice and consent banner before granting access. The XO login page does not present any banner text, and there is no acknowledgment/consent mechanism before login.

**Requirement (V-222434):** The application must display an organization-defined mandatory notice and consent banner before granting access.

**Requirement (V-222435):** The banner must remain on screen until the user **explicitly acknowledges** it (e.g., clicks "I Agree" or equivalent) before login proceeds. Simply displaying text that disappears automatically is not sufficient.

**Impact:**
- V-222434 returns **Open** — no banner text found on XO login page (curl scan of `https://localhost/` finds no required keywords)
...
1. **Nginx Reverse Proxy (Recommended for near-term):** Deploy nginx in front of XO with a custom `/consent` page. Users are redirected to the consent page before being proxied to XO. Nginx serves the banner HTML, and a session cookie records acknowledgment before allowing the XO login page to render. This approach does NOT require any XO code changes.
   ...
   location / {
       if ($cookie_consent_banner != "acknowledged") {
           return 302 /consent;
       }
...
2. **XO Native Banner (Long-term Vates feature request):** Add a configurable pre-login banner to the XO web UI. Configuration in `config.toml`:
   ...
   [loginBanner]
   enabled = true
   text = "Your organization's mandatory notice and consent banner text here..."
...
**Action Required from Vates:**
- **[NEAR-TERM]** Publish nginx reverse proxy configuration template that implements mandatory banner + acknowledgment for XO deployments
- **[LONG-TERM]** Add native configurable pre-login banner support to XO web UI with acknowledgment requirement
- **[DOCUMENTATION]** Include banner configuration in the XO hardening guide
- **[XOCE]** Recommend and document use of nginx as a required component for regulated XOCE deployments
...
| ARCH-004 | No FIPS 140-2 validation | Required for regulated environments | Unknown | Unknown | See Section 1.1 above |
| ARCH-005 | No SCAP benchmark available for XO | Manual verification required | N/A | N/A | Consider SCAP content development |
...
- V-206388: Install enterprise-approved SSL certificate
...
| V-207351 | Approved encryption algorithms | Open | Default SSH config not compliant |
...
3. **[CRITICAL] MFA Integration Guide** - CAC/PIV or hardware token integration for regulated environments (Section 1.5)
...
5. **[HIGH] Mandatory Banner Implementation** - Nginx consent page template OR native XO banner feature (Section 1.8)
...
9. **[HIGH] UTC Timezone Configuration** - XO deployment recommendation for regulated environments (Section 1.3)
...
| No mandatory consent banner | XO Login | ❌ OPEN | Nginx consent page or native XO banner feature |
...
| Centralized audit not configured | XO/XCP-ng Logs | ⚠️ SITE CONFIG | Configure remote syslog (XO: config.toml; XCP-ng: pool Advanced settings) |
...
|------|-----------|-----------------|
| Self-signed certificate | XO HTTPS | Deploy enterprise-approved SSL certificate |
...
**Framework:** Evaluate-STIG v1.2507.6 with Kismet Agbasi modifications
...
**Document Status:** ACTIVE - Updated March 15, 2026 (all 5 modules 100% complete — 1,047/1,047 functions)
