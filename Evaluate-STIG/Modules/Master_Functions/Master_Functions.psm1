...
Function Write-SummaryReport {
    ...
    Switch ($Platform) {
        ...
        "Linux" {
            # Modified by Kismet — fix broken lsblk/lvscan parsing, populate all 7 disk fields
            $DistinguishedName = "Not a domain member"
            $DiskIndex = 0
            $W32DiskDrive = @()
            Try {
                $LsblkOutput = $(lsblk -dno NAME,SIZE,MODEL,SERIAL,TRAN,TYPE 2>/dev/null)
                if ($LsblkOutput) {
                    foreach ($Line in ($LsblkOutput -split "`n")) {
                        $Line = $Line.Trim()
                        if ([string]::IsNullOrWhiteSpace($Line)) { continue }
                        $Fields = $Line -split '\s+', 6
                        if ($Fields.Count -ge 2) {
                            $W32DiskDrive += [ordered]@{
                                Index         = $DiskIndex.ToString()
                                DeviceID      = "/dev/$($Fields[0])"
                                Size          = $Fields[1]
                                Caption       = if ($Fields.Count -ge 3 -and $Fields[2]) { $Fields[2] } else { "N/A" }
                                SerialNumber  = if ($Fields.Count -ge 4 -and $Fields[3]) { $Fields[3] } else { "N/A" }
                                MediaType     = if ($Fields.Count -ge 6 -and $Fields[5]) { $Fields[5] } else { "disk" }
                                InterfaceType = if ($Fields.Count -ge 5 -and $Fields[4]) { $Fields[4].ToUpper() } else { "N/A" }
                            }
                            $DiskIndex++
                        }
                    }
            ...
            Catch {
                # Fallback: minimal disk info if lsblk fails
                $W32DiskDrive = @([ordered]@{
                    Index         = "0"
                    DeviceID      = "Unknown"
                    Size          = "Unknown"
            ...
            $ComputerData.DiskDrives = ($W32DiskDrive | ForEach-Object { @{'Disk' = [ordered]@{
                            Index         = $_.Index
                            DeviceID      = $_.DeviceID
                            Size          = $_.Size
                            Caption       = $_.Caption
    ...
    Try {
        Switch ($OSPlatform) {
            ...
            "Linux" {
                ...
                if ($NetAdapters) {
                    $NetAdapters = $NetAdapters | Where-Object { $_ -ne "lo" }
                }
                $NetInterfaces = New-Object System.Collections.Generic.List[System.Object]
                ForEach ($Adapter in $NetAdapters) {
                    ...
                    if (Test-Path /sys/class/net/$Adapter/ifindex){
                        ...
                        # MODIFIED_BY: Kismet Agbasi on 01/19/2026 - Fixed IPv4 address parsing (IP is in field 2, not 5/6)
                        $IPv4Address = (ip -4 addr show dev $Adapter | grep "inet " | awk '{print $2}' | cut -d '/' -f 1)
                        $IPv6Address = (ip -6 addr show dev $Adapter | grep "inet6 " | awk '{print $2}' | cut -d '/' -f 1)
                        $NewObj = [PSCustomObject]@{
...
Function Invoke-STIGScan {
    ...
    ForEach ($Vuln in $VulnsToScan) {
        ...
        Else {
            ...
            ElseIf ($PsModule -eq "Manual") {
                Write-Log -Path $LogPath -Message "    No automated check for $($Vuln.ID); marking Not_Reviewed (manual review required)." -Component $LogComponent -Type "Info" -OSPlatform $OSPlatform
                $Status = "Not_Reviewed"
                $FindingDetails = "Manual review required; no automated check implemented.`nRuleId: $RuleID`nGroupId: $GroupID`nSTIGID: $STIGID"
